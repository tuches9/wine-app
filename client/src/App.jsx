...
function App() {
  ...
  const loadScanIntoForm = (scan) => {
    setFormData(prev => ({ ...initialFormState, ...scan }));
  ...
  const handleImageChange = async (e) => {
    ...
    if (file.name.toLowerCase().endsWith('.heic') || file.name.toLowerCase().endsWith('.heif') || file.type === 'image/heic') {
      try {
        ...
        alert("הייתה שגיאה בהמרת קובץ ה-HEIC.");
    ...
    try {
      ...
      if (response.ok) {
        const newScan = {
          name: data.analyzedData.name || '',
          producer: data.analyzedData.producer || '',
          vintage: data.analyzedData.vintage || '',
          country: data.analyzedData.country || '',
          region: data.analyzedData.region || '',
          grapes: data.analyzedData.grapes || '',
          wineType: data.analyzedData.wineType || 'אדום',
          isNatural: data.analyzedData.isNatural || false,
          aiInsights: data.analyzedData.aiInsights || '',
          imageUrl: data.imageUrl,
          timestamp: Date.now() 
        };

        setRecentScans(prev => [newScan, ...prev]);
        loadScanIntoForm(newScan);
      }
    } catch (error) { alert('שגיאת תקשורת.'); } finally { setIsAnalyzing(false); }
  };
  ...
  // פונקציה חכמה למציאת דגל לפי שם המדינה בעברית
  const getCountryFlag = (country) => {
    ...
    return ''; // אם לא נמצאה מדינה מוכרת, לא נציג דגל כדי לשמור על מראה נקי
  };
